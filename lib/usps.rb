class Usps
  attr_accessor :usps_verification
  
  def initialize
    @usps_verification = ActiveShipping::USPS.new(login: ENV["USPS_LOGIN"], password: ENV["USPS_PASSWORD"], key: ENV["USPS_KEY"])
  end
  ...
  def find_rates(shipment_params)
    ...
    response = usps_verification.find_rates(origin, destination, packages)
